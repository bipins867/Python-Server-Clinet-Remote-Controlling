...
def genMdb():
    con=sqlite3.connect('C:\\MyDataBase\\main.db', check_same_thread=False)
    return con
...
class cb1:
    def __init__(self,mycursor,database):
        ...
        d='ATTACH DATABASE "C:\\MyDataBase\\{0}.db" as {1}'.format(self.database,self.database)
        
        try:
            #print(d)
            self.mycursor.execute(d)
        except:
    ...
    def showTablesByLike(self,like):
        ...
        d=d=self.mycursor.execute(sqlFormula)
        res=d.fetchall()
    ...
    def getColName(self,tableName):
        sqlFormula='pragma table_info("{0}")'.format(tableName)
        ...
        if(table is None or (not ms.inEqual(tableName,table))):
            ...
            d=self.mycursor.execute(sqlFormula)
            res=d.fetchall()
    ...
    def showTables(self):
        sqlFormula="SELECT name FROM sqlite_master WHERE type='table';"
        d=d=self.mycursor.execute(sqlFormula)
        res=d.fetchall()
        res=ms.modifySqlResult(res)
        return res
    ...
    def updateParticularData(self,tableName,field1,field2,word,value):
        sqlFormula='update '+tableName+ ' set '+  field2+ '="'+str(value)+'" where '+field1+'="'+str(word)+'"'
