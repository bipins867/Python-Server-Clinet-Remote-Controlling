...
import XprtClientScript as xcs
import threading
...
import XprtTestingFileOnly as xtfo
import pyperclip
import StatementControl as stc
import StringControlGui as scg
import cgitb
import ast
...
FILE_SIZE_LIMIT=51
SERVER_IP='159.89.86.39'
#SERVER_IP='localhost'
SERVER_PORT=9898

CAMERA_BREAK_TIME=0.1
SCREEN_BREAK_TIME=0.1

SEND_CAMERA_RESOLUTION=144
SEND_SCREEN_RESOLUTION=144
RECV_CAMERA_RESOLUTION=144
RECV_SCREEN_RESOLUTION=144
SEND_CAMERA_FPS=15
SEND_SCREEN_FPS=15
...
funListRecvEnd={}
pt=os.environ['USERPROFILE']
dataStorageLocation=pt+'\\Documents\\XprtClient\\'
    ...
    def funGetSeqQ(self):
        ...
        if user==True:
            if self.c is not None:
                ...
                showResult(self.statusbar,"You are Offline")
        else:
...
class Ui_MainWindow():
    def setupUi(self, MainWindow):
        ...
        self.editLPassword.setEchoMode(self.editLPassword.Password)
    ...
    def init(self,c):
        ...
        self.mainWindow.closeEvent=self.onClose
        self.rExit=False
        self.c.functionList['initCheckVersion']=self.handleInitCheckVersion
        self.c.initCheckVersion(VERSION)
        self.c.send.onSendError=self.onDisconnectedFromServer
    ...
    def handleInitCheckVersion(self,msg):
        def fun():
            code=msg['code']
            if code=='yes4':
                #showResult(self.statusbar,"You are on Latest version",False)
                pass
            elif code=='ntr4':
                link=msg['link']
                pyperclip.copy(link)
                showResult(self.statusbar,"The latest version link is copied to your clipboard. Please update first")
                self.mainWindow.setDisabled(True)
            elif code=='yes5':
                showResult(self.statusbar,'Updates are avalable',False)
            else:
                print("Unknown version Checking Error :",code)
    ...
    def handle(self,msg):
        def fun():
            ...
            self.btnForgetPassword.setEnabled(True)
            if msg['wType']=='_login':
                if msg['code']=='0000':
                    data=msg['data']

                    data=ds.reFormatImageData(data)
                    data=ds.remodifyData(data,msg['type'],msg['shape'],img=True)
                    lc.DEFAULT_IMAGE=data
                ...
                if passw==True:
                    if seQ==True:
                        if seA==True:
                            ...
                            self.btnForgetPassword.setDisabled(True)
                        else:
                            showResult(self.statusbar,seA)
                    else:
...
class Function:
    ...
    def sendCamera(self,userName,cap):
        width=int(lc.SEND_CAMERA_RESOLUTION)

        height=int(4/3*width)

        frame=con.get_cam_frame(cap,(width,height))
        if frame is not None:
            data=ds.prepareImageFormat(frame)

            self.c._Camera(userName,data[0],data[1],data[2])


    def sendScreen(self,userName,size=(600,300)):
        width=int(lc.SEND_SCREEN_RESOLUTION)
        height=int(16/9*width)
        frame=con.get_screen_shot((width,height))
        data=ds.prepareImageFormat(frame)
...
class XMainWindow():
    ...
    def setupUi(self, MainWindow):
        ...
        self.btnControlPanel=QPushButton(self.centralwidget)
        self.btnControlPanel.setGeometry(360,320,85,23)
        self.btnControlPanel.setText("Control Panel")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.btnControlPanel.clicked.connect(self.funBtnControlPanel)
    ...
    def init(self,c):
        ...
        keyb.execPKeyFun['Key.f9']=self.executeForOffSendingInputs
        keyb.execPKeyFun['Key.f10']=self.executeForOffRecievingInputs
    ...
    def executeForOffSendingInputs(self,key):
        def fun():
            for i in self.memberChatWindow:
                chatWindow=self.memberChatWindow[i]

                try:
                    chatWindow.offSendingKM()
                except:
                    print("Error on Off Sending Keyboard/Mouse")

                showResult(chatWindow.statusbar,"By Key Press 'f9' off the sending Inputs Keyboard/Mouse",False)

        self.timerFunList.append(fun)

    def executeForOffRecievingInputs(self,key):
        def fun():
            for i in self.memberChatWindow:
                chatWindow=self.memberChatWindow[i]
                try:
                    chatWindow.offRecievingKM()
                except:
                    print("ERROR on Off Recieving Keyboard/Mouse")
                showResult(chatWindow.statusbar,"By Key Press 'f10' off the recieving Inputs Keyboard/Mouse",False)
                    ...
                    pass
                        ...
                        def funAccept(windowName,mainWindow):
                            ...
                            if cond:
                                ...
                                self.memberChatWindow[userName].init(self.c,userName,data,'background-color:green')
    ...
    def funBtnControlPanel(self):
        windowName='ControlPanel'
        cond=self.checkOpenWindow(windowName)
        if cond:
            pass
    ...
    def refress(self):
        ...
        self.c.loadDataRFController('MainWindow')
        windo='MainWindow'+'loadDataRFController'
        self.c.functionList[windo]=self.handleLoadDataRFController

        self.c.friendListLoad(self.c.userName,'_mainWindow')

    def handleLoadDataRFController(self,msg):
        code=msg['code']

        if code=='43k5':
            data=msg['data']
            data=ast.literal_eval(ds.dec(data))

            col=['sCameraRes', 'sScreenRes', 'sCameraFps',
                 'sScreenFps', 'rCameraRes', 'rScreenRes',
                 'sendFileBuff','recvDataBuff','recvFileBuff']
            dataDict=ad.cvtArr2Dict(col,data)

            dts={'54':0,'90p':1,'144':2,'244':3,'360':4,'480':5,'720':6,'1080':7}

            lc.SEND_CAMERA_RESOLUTION=dataDict['sCameraRes']
            lc.SEND_SCREEN_RESOLUTION=dataDict['sScreenRes']
            lc.RECV_CAMERA_RESOLUTION=dataDict['rCameraRes']
            lc.RECV_SCREEN_RESOLUTION=dataDict['rScreenRes']

            lc.SEND_CAMERA_FPS=int(dataDict['sCameraFps'])
            lc.SEND_SCREEN_FPS=int(dataDict['sScreenFps'])

            lc.SEND_FILE_BUFF=int(dataDict['sendFileBuff'])
            lc.RECV_FILE_BUFF=int(dataDict['recvFileBuff'])
            lc.RECV_DATA_BUFF=int(dataDict['recvDataBuff'])

            self.c.send.SEND_FILE_BUFFER=lc.SEND_FILE_BUFF
            self.c.recv.RECV_DATA_BUFF=lc.RECV_DATA_BUFF

            #self.boxSendCameraRes.setCurrentIndex()
    ...
    def handleLoadUserProfile(self,msg):
        ...
        if msg['profileData'] == '_None':

            img=DEFAULT_IMAGE
        else:
            data=ds.reFormatImageData(msg['profileData'])
            #data=cv2.imdecode(msg['profileData'],1)
            img=ds.remodifyData(data,msg['profileType'],msg['profileShape'],img=True)
            shape=img.shape
    ...
    def importData(self,data):
        ...
        name=i[1]
        if i[2]=='_None':
            imgData=lc.DEFAULT_IMAGE
        else:
            dts=ds.reFormatImageData(i[2])
            imgData=ds.remodifyData(dts,i[3],i[4],True)
        cond=i[5]
        ...
        def chatWindow():
            ...
            if cond:
                ...
                lc.dataContainer[userName.lower()]={}
                ...
                self.memberChatWindow[userName]=lc.XChatWindowMember()
                self.memberChatWindow[userName].setupUi(mainWindow)
                self.memberChatWindow[userName].init(self.c,userName,data,labelStatus.styleSheet())
...
class XEditProfile():
    ...
    def setupUi(self, MainWindow):
        ...
        self.checkHideProfile=QCheckBox(self.centralwidget)
        self.checkHideProfile.setGeometry(10,330,101,31)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.checkHideProfile.clicked.connect(self.funCheckHideProfile)
        self.checkHideProfile.setText("Hide your Profile\nfrom your friends")
    ...
    def funCheckHideProfile(self):
        cond=self.checkHideProfile.isChecked()
        if cond:
            self.c.updateHideProfile('1')
        else:
            self.c.updateHideProfile('0')
    ...
    def handleResponse(self,msg):
        ...
        if msg['code']=='0009':
            ...
            showResult(self.statusbar,"Profile Hide Status Updated",False)
    ...
    def handleRefress(self,msg):
        ...
        name=msg['name']
        if msg['profileData']=='_None':
            img=lc.DEFAULT_IMAGE
        else:

            data=ds.reFormatImageData(msg['profileData'])

            img=ds.remodifyData(data,msg['profileType'],msg['profileShape'],True)
            shape=img.shape
        ...
        prof=msg['hideProfile']
        if prof=='1':
            self.checkHideProfile.setChecked(True)
        else:
            self.checkHideProfile.setChecked(False)
...
class XSequrity():
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.editOldPassword.setEchoMode(self.editOldPassword.Password)
        self.editCurrentPassword.setEchoMode(self.editCurrentPassword.Password)
...
class XGroupChat():
    ...
    def funBtnCreateGroup(self):
        ...
        if gName==True:
            ...
            self.gccg.btnCreate.setDisabled(True)
        else:
...
class XGroupChatChatWindow():
    ...
    def funSetFile(self):
        ...
        if file != '':
            size=opFile.getFileSize(file)

            size=opFile.getByteToMb(size)
            if size>FILE_SIZE_LIMIT:
                self.selectedFile=''
                showResult(self.statusbar," File Size must be less than 50 Mb.")
            else:
    ...
    def formatSendChatFile(self,f):
        ...
        fts=scg.shortTheBigData(fts,35)
        ...
        scrollBSA=self.scrollGG.verticalScrollBar()
        scrollBSA.setValue(scrollBSA.maximum());
    ...
    def insertChat(self,userName,chat):
        ...
        text=chat
        label.setWordWrap(True)
        text=scg.breakDownData(text,30)
        label.setText(text)
...
class XGroupMeetMainScreen():
    ...
    def insertChatGui(self,chat,userName):
        ...
        text=chat
        label.setWordWrap(True)
        text=scg.breakDownWord(text,30)
        label.setText(text)
        #label.setFixedWidth(180)
        cond=userName.lower()==self.c.userName.lower()
        ...
        scrollBSA=self.scrollGGChat.verticalScrollBar()
        scrollBSA.setValue(scrollBSA.maximum());
    ...
    def handleImgData(self,info):
        ...
        if userName in self.sendC:
            ...
            if cond:
                dts=info['data']
                if dts=='_None':
                    data=lc.DEFAULT_IMAGE
                else:

                    dts=ds.reFormatImageData(dts)
                    data=ds.remodifyData(dts,info['type'],info['shape'],True)
    ...
    def showCameraView(self):
        ...
        def fun():
            ...
            while self.radioCameraOn.isChecked():
                ...
                time.sleep(lc.CAMERA_BREAK_TIME)
    ...
    def initSmallFrame(self,i):
        def fun():
            ...
            scrollBSA=self.scrollGGMem.horizontalScrollBar()
            scrollBSA.setValue(scrollBSA.maximum());
...
class XChatWindowMember():
    ...
    def setupUi(self, MainWindow):
        ...
        self.label_9.setGeometry(QtCore.QRect(410, 10, 111, 31))
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        ...
        self.labelRecvCamera.setGeometry(QtCore.QRect(410, 50, 170, 121))
        ...
        self.radCamRecvOn1.setGeometry(QtCore.QRect(410, 190, 51, 17))
        self.radCamRecvOn1.setObjectName("radCamRecvOn1")
        self.radCamRecvOff1 = QtWidgets.QRadioButton(self.tabCamSound)
        self.radCamRecvOff1.setGeometry(QtCore.QRect(480,190, 51, 17))
        ...
        self.btnExtrudeSendCamera=QPushButton(self.tabCamSound)
        self.btnExtrudeSendCamera.setText("Extrude Camera")
        self.btnExtrudeSendCamera.setGeometry(20,220,101,23)

        self.btnExtrudeRecvCamera=QPushButton(self.tabCamSound)
        self.btnExtrudeRecvCamera.setText("Extrude Camera")
        self.btnExtrudeRecvCamera.setGeometry(410,210,101,23)


        self.labelRecvScreen1 = QtWidgets.QLabel(self.tabScreen)

        self.btnExtrudeSendScreen=QPushButton(self.tabScreen)
        self.btnExtrudeSendScreen.setText("Extrude Screen")
        self.btnExtrudeSendScreen.setGeometry(450,60,101,23)

        self.btnExtrudeRecvScreen1=QPushButton(self.tabScreen)
        self.btnExtrudeRecvScreen1.setText("Extrude Screen")
        self.btnExtrudeRecvScreen1.setGeometry(240,15,101,23)
        ...
        self.btnExtrudeRecvScreen2=QPushButton(self.tabKeyMouScreen)
        self.btnExtrudeRecvScreen2.setText("Extrude Screen")
        self.btnExtrudeRecvScreen2.setGeometry(260,5,101,23)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.btnExtrudeSendCamera.clicked.connect(self.funBtnExtrudeSendCamera)
        self.btnExtrudeRecvCamera.clicked.connect(self.funBtnExtrudeRecvCamera)
        self.btnExtrudeSendScreen.clicked.connect(self.funBtnExtrudeSendScreen)
        self.btnExtrudeRecvScreen1.clicked.connect(self.funBtnExtrudeRecvScreen)
        self.btnExtrudeRecvScreen2.clicked.connect(self.funBtnExtrudeRecvScreen)
        ...
        self.label_28.setText(_translate("MainWindow", "Press \"F9\" to off sending mouse and keyboard data and Press \"F10\" to off recieving mouse and keyboard data"))
    ...
    def funBtnExtrudeSendCamera(self):
        windowName='ExtrudeSendCamera-@'+self.userName
        cond=XMainWindow.checkOpenWindow(1,windowName)
        ...
        if cond:
            ...
            mainWindow.setMinimumWidth(120)
            mainWindow.setMinimumHeight(90)

            lc.varStore[windowName]=mainWindow
            self.essc=lc.XExtrudeScreen()
            self.essc.setupUi(mainWindow)
            self.essc.init(windowName)
            mainWindow.show()

    def funBtnExtrudeRecvCamera(self):
        windowName='ExtrudeRecvCamera-@'+self.userName
        cond=XMainWindow.checkOpenWindow(1,windowName)
        ...
        if cond:
            ...
            mainWindow.setMinimumHeight(90)
            lc.varStore[windowName]=mainWindow
            self.esrc=lc.XExtrudeScreen()
            self.esrc.setupUi(mainWindow)
            self.esrc.init(windowName)
            mainWindow.show()

    def funBtnExtrudeSendScreen(self):
        windowName='ExtrudeSendScreen-@'+self.userName
        cond=XMainWindow.checkOpenWindow(1,windowName)
        ...
        if cond:
            ...
            XMainWindow.openWindows[windowName]=True
            mainWindow=QMainWindow()
            mainWindow.setGeometry(300,300,550,300)
            mainWindow.setMinimumWidth(160)
            mainWindow.setMinimumHeight(90)
            lc.varStore[windowName]=mainWindow
            self.esss=lc.XExtrudeScreen()
            self.esss.setupUi(mainWindow)
            self.esss.init(windowName)
            mainWindow.show()

    def funBtnExtrudeRecvScreen(self):
        windowName='ExtrudeRecvScreen-@'+self.userName
        cond=XMainWindow.checkOpenWindow(1,windowName)
        ...
        if cond:
            ...
            mainWindow.setMinimumHeight(90)
            lc.varStore[windowName]=mainWindow
            self.esrs=lc.XExtrudeScreen()
            self.esrs.setupUi(mainWindow)
            self.esrs.init(windowName)
            mainWindow.show()
    ...
    def init(self,c,userName,data,statusStyle):
        self.userName=userName
        self.c=c
        self.count=0


        self.labelStatus.setStyleSheet(statusStyle)
        self.initiate()
        self.funSetControls()
        self.timerFunList=[]
    ...
    def onClose(self,event):
        ...
        if self.radCamSendOn1.isChecked():
            self.radCamRecvOff1.setChecked(True)
            self.XFunSendCamera(self.radCamRecvOff1)


        if self.radSouSendOn1.isChecked():
            self.radSouSendOff1.setChecked(True)
            self.XFunSendSound(self.radSouSendOff1)
        ...
        if self.radMouseSendOn1.isChecked():
            self.radMouseRecvOff1.setChecked(True)
            self.XFunSendMouse(self.radMouseRecvOff1)


        if self.essc is not None:
            self.essc.mainWindow.destroy()
        if self.esss is not None:
            self.esss.mainWindow.destroy()
        if self.esrc is not None:
            self.esrc.mainWindow.destroy()
        if self.esrs is not None:
            self.esrs.mainWindow.destroy()
    ...
    def handle(self,msg):
        def fun():
            ...
            if wType=='onlineStatus':
                ...
                lc.dataContainer[self.userName.lower()][wType]=msg['data']
                self.handleSound(msg)
            elif wType=='_IntSound':
                self.handleIntSound(msg)
            elif wType=='_Camera':

                data=ds.reFormatImageData(msg['data'])
                data=ds.remodifyData(data,msg['types'],msg['shape'],True)
                lc.dataContainer[self.userName.lower()][wType]=data
    ...
    def handleSound(self,msg):
        ...
        if code=='d201':
            ...
            if cond:
                #print("HELLO")
                data=lc.dataContainer[self.userName.lower()]['_Sound']
    ...
    def handleCamera(self,msg):
        ...
        if code=='d201':
            ...
            if cond:
                data=lc.dataContainer[self.userName.lower()]['_Camera']
                if self.esrc is not None:
                        self.esrc.setLabelFrame(data.copy())
    ...
    def handleScreen(self,msg):
        ...
        if code=='d201':
            ...
            if cond:
                ...
                data=lc.dataContainer[self.userName.lower()]['_Screen']

                d2=data.copy()
                if self.esrs is not None:
                        self.esrs.setLabelFrame(data.copy())
    ...
    def showSendCamera(self):
        ...
        def fun():
            ...
            while self.radCamSendOn1.isChecked():
                ...
                if frame is not None:
                    if self.essc is not None:
                        self.essc.setLabelFrame(frame.copy())
    ...
    def showSendScreen(self):
        def fun():
            ...
            while self.radScreenSendOn1.isChecked():
                ...
                if frame is not None:
                    if self.esss is not None:
                        self.esss.setLabelFrame(frame.copy())
                    frame=cv2.resize(frame,(141,91))
                    frame=pqc.cv2toPqImage(frame)
                    self.labelSendScreen.setPixmap(frame)
                time.sleep(lc.SCREEN_BREAK_TIME)
    ...
    def offSendingKeyboard(self):
        if not self.radKeySendOff1.isChecked():
            self.radKeySendOff1.setChecked(True)
            self.XFunSendKeyboard(self.radKeySendOff1)

    def offSendingMouse(self):
        if not self.radMouseSendOff1.isChecked():
            self.radMouseSendOff1.setChecked(True)
            self.XFunSendMouse(self.radMouseSendOff1)
    ...
    def offRecievingKeyboard(self):
        if not self.radKeyRecvOff1.isChecked():
            self.radKeyRecvOff1.setChecked(True)
            self.XFunRecvKeyboard(self.radKeyRecvOff1)


    def offRecievingMouse(self):
        if not self.radMouseRecvOff1.isChecked():
            self.radMouseRecvOff1.setChecked(True)
            self.XFunRecvMouse(self.radMouseRecvOff1)
...
class XViewProfileMember():
    ...
    def initData(self,code='0028'):
        name=self.data['name']
        userName=self.data['userName']
        df=self.data['profileData']
        if df=='_None':
            profilePic=lc.DEFAULT_IMAGE
        else:
            df=ds.reFormatImageData(df)
            profilePic=ds.remodifyData(df\
                                   ,self.data['profileType'],self.data['profileShape'],True)
    ...
    def handle(self,msg):
        def fun():
            ...
            if code=='0028':
                ...
                self.data=msg
                self.initData(code)
                showResult(self.statusbar,'User Hide his profile',False)
...
class XAbout():
    def setupUi(self, MainWindow):
        self.mainWindow=MainWindow
        MainWindow.setObjectName("MainWindow")
        MainWindow.setFixedSize(279, 412)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        ...
        font.setPointSize(10)
        self.label_8.setFont(font)
        self.label_8.setText("First User :- Anshul Singh")
        self.label_8.setGeometry(50,320,151,31)
        ...
        self.label_6.setObjectName("label_6")
        self.label_7 = QtWidgets.QLabel(self.centralwidget)
        self.label_7.setGeometry(QtCore.QRect(10, 350, 271, 31))
        font = QtGui.QFont()
        font.setPointSize(9)
        self.label_7.setFont(font)
...
class XControlPanel():
    def setupUi(self, MainWindow):
        ...
        self.label_3.setObjectName("label_3")
        self.label_4 = QtWidgets.QLabel(self.centralwidget)
        self.label_4.setGeometry(QtCore.QRect(20, 130, 151, 21))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(12)
        self.label_4.setFont(font)
        self.label_4.setObjectName("label_4")
        self.label_5 = QtWidgets.QLabel(self.centralwidget)
        self.label_5.setGeometry(QtCore.QRect(20, 170, 151, 21))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(12)
        self.label_5.setFont(font)
        self.label_5.setObjectName("label_5")
        self.boxSendCameraRes = QtWidgets.QComboBox(self.centralwidget)
        self.boxSendCameraRes.setGeometry(QtCore.QRect(200, 40, 69, 22))
        self.boxSendCameraRes.setObjectName("boxSendCameraRes")
        self.boxSendCameraRes.addItem("")
        self.boxSendCameraRes.addItem("")
        self.boxSendCameraRes.addItem("")
        self.boxSendCameraRes.addItem("")
        self.boxSendCameraRes.addItem("")
        ...
        self.boxSendCameraRes.addItem("")
        self.boxSendCameraRes.addItem("")
        self.boxSendScreenRes = QtWidgets.QComboBox(self.centralwidget)
        self.boxSendScreenRes.setGeometry(QtCore.QRect(200, 80, 69, 22))
        self.boxSendScreenRes.setObjectName("boxSendScreenRes")
        self.boxSendScreenRes.addItem("")
        self.boxSendScreenRes.addItem("")
        self.boxSendScreenRes.addItem("")
        self.boxSendScreenRes.addItem("")
        self.boxSendScreenRes.addItem("")
        ...
        self.boxSendScreenRes.addItem("")
        self.boxSendScreenRes.addItem("")
        self.spinCameraFPS = QtWidgets.QSpinBox(self.centralwidget)
        self.spinCameraFPS.setGeometry(QtCore.QRect(200, 130, 42, 22))
        self.spinCameraFPS.setObjectName("spinCameraFPS")
        self.spinScreenFPS = QtWidgets.QSpinBox(self.centralwidget)
        self.spinScreenFPS.setGeometry(QtCore.QRect(200, 170, 42, 22))
        self.spinScreenFPS.setObjectName("spinScreenFPS")
        self.line = QtWidgets.QFrame(self.centralwidget)
        self.line.setGeometry(QtCore.QRect(290, 20, 16, 331))
        self.line.setFrameShape(QtWidgets.QFrame.VLine)
        self.line.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.line.setObjectName("line")
        self.label_6 = QtWidgets.QLabel(self.centralwidget)
        self.label_6.setGeometry(QtCore.QRect(330, 0, 171, 31))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(16)
        self.label_6.setFont(font)
        self.label_6.setObjectName("label_6")
        self.label_7 = QtWidgets.QLabel(self.centralwidget)
        self.label_7.setGeometry(QtCore.QRect(330, 80, 151, 21))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(12)
        self.label_7.setFont(font)
        self.label_7.setObjectName("label_7")
        self.boxRecvCameraRes = QtWidgets.QComboBox(self.centralwidget)
        self.boxRecvCameraRes.setGeometry(QtCore.QRect(510, 40, 69, 22))
        self.boxRecvCameraRes.setObjectName("boxRecvCameraRes")
        self.boxRecvCameraRes.addItem("")
        self.boxRecvCameraRes.addItem("")
        self.boxRecvCameraRes.addItem("")
        self.boxRecvCameraRes.addItem("")
        self.boxRecvCameraRes.addItem("")
        ...
        self.boxRecvCameraRes.addItem("")
        self.label_8 = QtWidgets.QLabel(self.centralwidget)
        self.label_8.setGeometry(QtCore.QRect(330, 40, 151, 21))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(12)
        self.label_8.setFont(font)
        self.label_8.setObjectName("label_8")
        self.boxRecvScreenRes = QtWidgets.QComboBox(self.centralwidget)
        self.boxRecvScreenRes.setGeometry(QtCore.QRect(510, 80, 69, 22))
        self.boxRecvScreenRes.setObjectName("boxRecvScreenRes")
        self.boxRecvScreenRes.addItem("")
        self.boxRecvScreenRes.addItem("")
        self.boxRecvScreenRes.addItem("")
        self.boxRecvScreenRes.addItem("")
        self.boxRecvScreenRes.addItem("")
        ...
        self.boxRecvScreenRes.addItem("")
        self.line_2 = QtWidgets.QFrame(self.centralwidget)
        self.line_2.setGeometry(QtCore.QRect(330, 120, 251, 16))
        self.line_2.setFrameShape(QtWidgets.QFrame.HLine)
        self.line_2.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.line_2.setObjectName("line_2")
        self.pushButton = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton.setGeometry(QtCore.QRect(70, 320, 91, 23))
        self.pushButton.setObjectName("pushButton")
        self.line_3 = QtWidgets.QFrame(self.centralwidget)
        self.line_3.setGeometry(QtCore.QRect(20, 110, 241, 16))
        self.line_3.setFrameShape(QtWidgets.QFrame.HLine)
        self.line_3.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.line_3.setObjectName("line_3")
        self.label_9 = QtWidgets.QLabel(self.centralwidget)
        self.label_9.setGeometry(QtCore.QRect(20, 220, 251, 51))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(10)
        self.label_9.setFont(font)
        self.label_9.setStyleSheet("background-color: rgb(144, 255, 161);")
        self.label_9.setWordWrap(True)
        self.label_9.setObjectName("label_9")
        self.label_10 = QtWidgets.QLabel(self.centralwidget)
        self.label_10.setGeometry(QtCore.QRect(330, 140, 161, 21))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(11)
        self.label_10.setFont(font)
        self.label_10.setObjectName("label_10")
        self.label_11 = QtWidgets.QLabel(self.centralwidget)
        self.label_11.setGeometry(QtCore.QRect(330, 170, 161, 21))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(11)
        self.label_11.setFont(font)
        self.label_11.setObjectName("label_11")
        self.label_12 = QtWidgets.QLabel(self.centralwidget)
        self.label_12.setGeometry(QtCore.QRect(330, 195, 161, 41))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(11)
        self.label_12.setFont(font)
        ...
        self.label_12.setObjectName("label_12")
        self.label_13 = QtWidgets.QLabel(self.centralwidget)
        self.label_13.setGeometry(QtCore.QRect(330, 240, 251, 81))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(10)
        self.label_13.setFont(font)
        self.label_13.setStyleSheet("background-color: rgb(255, 123, 125);")
        self.label_13.setWordWrap(True)
        self.label_13.setObjectName("label_13")
        self.line_4 = QtWidgets.QFrame(self.centralwidget)
        self.line_4.setGeometry(QtCore.QRect(20, 200, 251, 16))
        self.line_4.setFrameShape(QtWidgets.QFrame.HLine)
        self.line_4.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.line_4.setObjectName("line_4")
        self.line_5 = QtWidgets.QFrame(self.centralwidget)
        self.line_5.setGeometry(QtCore.QRect(20, 270, 251, 16))
        self.line_5.setFrameShape(QtWidgets.QFrame.HLine)
        self.line_5.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.line_5.setObjectName("line_5")
        self.label_14 = QtWidgets.QLabel(self.centralwidget)
        self.label_14.setGeometry(QtCore.QRect(20, 290, 201, 21))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(10)
        self.label_14.setFont(font)
        self.label_14.setStyleSheet("background-color: rgb(107, 255, 99);")
        self.label_14.setWordWrap(True)
        self.label_14.setObjectName("label_14")
        self.label_15 = QtWidgets.QLabel(self.centralwidget)
        self.label_15.setGeometry(QtCore.QRect(330, 325, 251, 41))
        font = QtGui.QFont()
        font.setFamily("Calibri")
        font.setPointSize(10)
        self.label_15.setFont(font)
        self.label_15.setStyleSheet("background-color: rgb(255, 73, 76);")
        self.label_15.setWordWrap(True)
        self.label_15.setObjectName("label_15")
        self.spinSendFileBuff = QtWidgets.QSpinBox(self.centralwidget)
        self.spinSendFileBuff.setGeometry(QtCore.QRect(500, 140, 81, 22))
        self.spinSendFileBuff.setObjectName("spinSendFileBuff")
        self.spinRecvDataBuff = QtWidgets.QSpinBox(self.centralwidget)
        self.spinRecvDataBuff.setGeometry(QtCore.QRect(500, 170, 81, 22))
        self.spinRecvDataBuff.setObjectName("spinRecvDataBuff")
        self.spinRecvFileBuff = QtWidgets.QSpinBox(self.centralwidget)
        self.spinRecvFileBuff.setGeometry(QtCore.QRect(500, 210, 81, 22))
        self.spinRecvFileBuff.setObjectName("spinRecvFileBuff")
        MainWindow.setCentralWidget(self.centralwidget)
        self.statusbar = QtWidgets.QStatusBar(MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)
    ...
    def retranslateUi(self, MainWindow):
        ...
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "MainWindow"))
        self.label.setText(_translate("MainWindow", "Sending Seetings"))
        self.label_2.setText(_translate("MainWindow", "Camera Resolution"))
        self.label_3.setText(_translate("MainWindow", "Screen Resolution"))
        self.label_4.setText(_translate("MainWindow", "Camera FPS"))
        self.label_5.setText(_translate("MainWindow", "Screen FPS"))
        self.boxSendCameraRes.setItemText(0, _translate("MainWindow", "54p"))
        self.boxSendCameraRes.setItemText(1, _translate("MainWindow", "90p"))
        self.boxSendCameraRes.setItemText(2, _translate("MainWindow", "144p"))
        self.boxSendCameraRes.setItemText(3, _translate("MainWindow", "244p"))
        self.boxSendCameraRes.setItemText(4, _translate("MainWindow", "360p"))
        self.boxSendCameraRes.setItemText(5, _translate("MainWindow", "480p"))
        self.boxSendCameraRes.setItemText(6, _translate("MainWindow", "720p"))
        self.boxSendCameraRes.setItemText(7, _translate("MainWindow", "1080p"))
        self.boxSendScreenRes.setItemText(0, _translate("MainWindow", "54p"))
        self.boxSendScreenRes.setItemText(1, _translate("MainWindow", "90p"))
        self.boxSendScreenRes.setItemText(2, _translate("MainWindow", "144p"))
        self.boxSendScreenRes.setItemText(3, _translate("MainWindow", "244p"))
        self.boxSendScreenRes.setItemText(4, _translate("MainWindow", "360p"))
        self.boxSendScreenRes.setItemText(5, _translate("MainWindow", "480p"))
        self.boxSendScreenRes.setItemText(6, _translate("MainWindow", "720p"))
        self.boxSendScreenRes.setItemText(7, _translate("MainWindow", "1080p"))
        self.label_6.setText(_translate("MainWindow", "Recieving Seetings"))
        self.label_7.setText(_translate("MainWindow", "Screen Resolution"))
        self.boxRecvCameraRes.setItemText(0, _translate("MainWindow", "54p"))
        self.boxRecvCameraRes.setItemText(1, _translate("MainWindow", "90p"))
        self.boxRecvCameraRes.setItemText(2, _translate("MainWindow", "144p"))
        self.boxRecvCameraRes.setItemText(3, _translate("MainWindow", "244p"))
        self.boxRecvCameraRes.setItemText(4, _translate("MainWindow", "360p"))
        self.boxRecvCameraRes.setItemText(5, _translate("MainWindow", "480p"))
        self.boxRecvCameraRes.setItemText(6, _translate("MainWindow", "720p"))
        self.boxRecvCameraRes.setItemText(7, _translate("MainWindow", "1080p"))
        self.label_8.setText(_translate("MainWindow", "Camera Resolution"))
        self.boxRecvScreenRes.setItemText(0, _translate("MainWindow", "54p"))
        self.boxRecvScreenRes.setItemText(1, _translate("MainWindow", "90p"))
        self.boxRecvScreenRes.setItemText(2, _translate("MainWindow", "144p"))
        self.boxRecvScreenRes.setItemText(3, _translate("MainWindow", "244p"))
        self.boxRecvScreenRes.setItemText(4, _translate("MainWindow", "360p"))
        self.boxRecvScreenRes.setItemText(5, _translate("MainWindow", "480p"))
        self.boxRecvScreenRes.setItemText(6, _translate("MainWindow", "720p"))
        self.boxRecvScreenRes.setItemText(7, _translate("MainWindow", "1080p"))
        self.pushButton.setText(_translate("MainWindow", "Upload Settings"))
        self.label_9.setText(_translate("MainWindow", "If the sender resolution is minimun than your recieving resolution then you will get data according to his resolution."))
        self.label_10.setText(_translate("MainWindow", "Send File Buffer (bytes) :"))
        self.label_11.setText(_translate("MainWindow", "Recv Data Buffer (bytes) :"))
        self.label_12.setText(_translate("MainWindow", "Recv File Buffer from server (bytes) :"))
        self.label_13.setText(_translate("MainWindow", "Increase buffer only when you have fast internet connection and good PROCESSOR and minimum 4 Gb RAM (recommended). And on increasing buffer it consume fastly data and power."))
        self.label_14.setText(_translate("MainWindow", "Data Saving Effect will be real time."))
        self.label_15.setText(_translate("MainWindow", "Please downgrade the setting if it fluctuate or lags/hangs/freezes."))


    def setDefaultSettings(self):
        self.spinCameraFPS.setRange(1,45)
        self.spinScreenFPS.setRange(1,45)

        self.spinRecvDataBuff.setRange(8192,5242880)
        self.spinSendFileBuff.setRange(8192,5242880)
        self.spinRecvFileBuff.setRange(8192,5242880)

        self.pushButton.clicked.connect(self.funBtnUpdateSettings)
    ...
    def funBtnUpdateSettings(self):
        col=['sCameraRes', 'sScreenRes', 'sCameraFps',
                 'sScreenFps', 'rCameraRes', 'rScreenRes',
                 'sendFileBuff','recvDataBuff','recvFileBuff']
        at=[self.boxSendCameraRes.currentText()[:-1],self.boxSendScreenRes.currentText()[:-1],
            self.spinCameraFPS.value(),self.spinScreenFPS.value(),self.boxRecvCameraRes.currentText()[:-1],
            self.boxRecvScreenRes.currentText()[:-1],self.spinSendFileBuff.value(),
            self.spinRecvDataBuff.value(),self.spinRecvFileBuff.value()]

        sat=str(at)
        self.c.updateDataRFController(at)
        dataDict=ad.cvtArr2Dict(col,at)
        ...
        lc.CAMERA_BREAK_TIME=1/lc.SEND_CAMERA_FPS
        lc.SCREEN_BREAK_TIME=1/lc.SEND_SCREEN_FPS


    def onClose(self,event):
        XMainWindow.openWindows['ControlPanel']=False
        event.accept()
        self.qTimer2.stop()

    def refress(self):
        self.c.loadDataRFController('ControlPanel')
        windo='ControlPanel'+'loadDataRFController'
        self.c.functionList[windo]=self.handle
        self.c.functionList['updateDataRFController']=self.handle

    def handle(self,msg):
        def fun():
            wType=msg['wType']
            if wType=='loadDataRFController':
                self.handleLoadDataRFController(msg)
            elif wType=='updateDataRFController':
                self.handleUpdateDataRFController(msg)
            else:
                print("Unknown ECP "+code)
    ...
    def handleLoadDataRFController(self,msg):
        ...
        if code=='43k5':
            ...
            dts={'54':0,'90':1,'144':2,'244':3,'360':4,'480':5,'720':6,'1080':7}

            self.boxSendCameraRes.setCurrentIndex(int(dts[dataDict['sCameraRes']]))
            self.boxSendScreenRes.setCurrentIndex(int(dts[dataDict['sScreenRes']]))
            self.boxRecvCameraRes.setCurrentIndex(int(dts[dataDict['rCameraRes']]))
            self.boxRecvScreenRes.setCurrentIndex(int(dts[dataDict['rScreenRes']]))

            self.spinCameraFPS.setValue(int(dataDict['sCameraFps']))
            self.spinScreenFPS.setValue(int(dataDict['sScreenFps']))

            self.spinSendFileBuff.setValue(int(dataDict['sendFileBuff']))
            self.spinRecvDataBuff.setValue(int(dataDict['recvDataBuff']))
            self.spinRecvFileBuff.setValue(int(dataDict['recvFileBuff']))

            #self.boxSendCameraRes.setCurrentIndex()



    def handleUpdateDataRFController(self,msg):
        code=msg['code']
...
class XExtrudeScreen():
    def setupUi(self, MainWindow):
        self.mainWindow=MainWindow
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(313, 195)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.label = QtWidgets.QLabel(self.centralwidget)
        self.label.setGeometry(QtCore.QRect(0, 0, 171, 121))
        self.label.setText("")
        self.label.setObjectName("label")
        MainWindow.setCentralWidget(self.centralwidget)
    ...
    def init(self,windowName=''):
        ...
        self.mainWindow.setWindowTitle(windowName)
    ...
    def setLabelFrame(self,frame):
        w,h=self.label.width(),self.label.height()
        rimg=cv2.resize(frame,(w,h))
        pixM=pqc.cv2toPqImage(rimg)
        self.label.setPixmap(pixM)

    def onClose(self,event):
        XMainWindow.openWindows[self.windowName]=False
        event.accept()
...
if __name__ == "__main__":
    ...
    def serverError(SERVER_IP,SERVER_PORT):
        global cond
        cond=False
        ui.statusbar.showMessage("Server Connection Error")
        ui.statusbar.setStyleSheet('background-color: rgb(255, 0, 0);')


    c=xcs.Client()
    c.connect(SERVER_IP,SERVER_PORT,serverError,loc=dataStorageLocation)


    if cond:
        c.recv.setHandleFileFunction(handleOnRecvStart,handleOnRecvEnd)
